...
static u64 flip_A1(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r, flipped = 0ULL;
...
static u64 flip_B1(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C1(const u64 discs_player, const u64 discs_opponent) {
	
	u64 my_discs, opp_discs, r;
	
	/* direction _W */
	u64 flipped = (discs_opponent & 0X4000000000000000ULL) & (discs_player >> 1);
...
static u64 flip_D1(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_E1(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_F1(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	
	/* direction _E */
	u64 flipped =(discs_opponent & 0X0200000000000000ULL) & (discs_player << 1);
...
static u64 flip_G1(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H1(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_A2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_B2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C2(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SW */
	u64 flipped = (discs_opponent & 0X0000400000000000ULL) & (discs_player << 7);
...
static u64 flip_D2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_E2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_F2(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction _E */
	u64 flipped = (discs_opponent & 0X0002000000000000ULL) & (discs_player << 1);
...
static u64 flip_G2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H2(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_A3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction N_ */
	u64 flipped = (discs_opponent & 0X0080000000000000ULL) & (discs_player >> 8);
...
static u64 flip_B3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NE */
	u64 flipped = (discs_opponent & 0X0020000000000000ULL) & (discs_player >> 7);
...
static u64 flip_C3(const u64 discs_player, const u64 discs_opponent) {
	
	u64 my_discs, opp_discs, r;
		
	/* direction NE */
	u64 flipped = (discs_opponent & 0X0010000000000000ULL) & (discs_player >> 7);
	/* direction N_ */
...
static u64 flip_D3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NE */
	u64 flipped = (discs_opponent & 0X0008000000000000ULL) & (discs_player >> 7);
...
static u64 flip_E3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NE */
	u64 flipped = (discs_opponent & 0X0004000000000000ULL) & (discs_player >> 7);
...
static u64 flip_F3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000200000000ULL) & (discs_player << 9);
...
static u64 flip_G3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NW */
	u64 flipped = (discs_opponent & 0X0004000000000000ULL) & (discs_player >> 9);
...
static u64 flip_H3(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;

	/* direction NW */
	u64 flipped = (discs_opponent & 0X0002000000000000ULL) & (discs_player >> 9);
...
static u64 flip_A4(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_B4(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C4(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NW */
	u64 flipped = (discs_opponent & 0X0000400000000000ULL) & (discs_player >> 9);
...
static u64 flip_F4(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction _E */
	u64 flipped = (discs_opponent & 0X0000000200000000ULL) & (discs_player << 1);
...
static u64 flip_G4(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H4(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_A5(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_B5(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C5(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NW */
	u64 flipped = (discs_opponent & 0X0000004000000000ULL) & (discs_player >> 9);
...
static u64 flip_F5(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000000020000ULL) & (discs_player << 9);
...
static u64 flip_G5(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H5(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_A6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped =(discs_opponent & 0X0000000000004000ULL) & (discs_player << 9);
...
static u64 flip_B6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction S */
	u64 flipped = (discs_opponent & 0X0000000000004000ULL) & (discs_player << 8);
...
static u64 flip_C6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000000001000ULL) & (discs_player << 9);
...
static u64 flip_D6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000000000800ULL) & (discs_player << 9);
...
static u64 flip_E6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000000000400ULL) & (discs_player << 9);
...
static u64 flip_F6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SE */
	u64 flipped = (discs_opponent & 0X0000000000000200ULL) & (discs_player << 9);
...
static u64 flip_G6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SW */
	u64 flipped = (discs_opponent & 0X0000000000000400ULL) & (discs_player << 7);
...
static u64 flip_H6(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction SW */
	u64 flipped = (discs_opponent & 0X0000000000000200ULL) & (discs_player << 7);
...
static u64 flip_A7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_B7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C7(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction NW */
	u64 flipped = (discs_opponent & 0X0000000000400000ULL) & (discs_player >> 9);
...
static u64 flip_D7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_E7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_F7(const u64 discs_player, const u64 discs_opponent) {

	u64 my_discs, opp_discs, r;
	
	/* direction _E */
	u64 flipped = (discs_opponent & 0X0000000000000200ULL) & (discs_player << 1);
...
static u64 flip_G7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H7(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_A8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_B8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_C8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_D8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_E8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_F8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_G8(const u64 discs_player, const u64 discs_opponent) {
...
static u64 flip_H8(const u64 discs_player, const u64 discs_opponent) {
